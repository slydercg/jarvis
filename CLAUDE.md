...
| `bridge/tickets.mjs` / `maillinks.mjs` / `src/lib/tickets.ts` | Ticket links, built only from known Jira/ADO sites, and Outlook links for Protective brief lines, built only from a message id matched in the mailbox; `tickets.ts` is the page's own check (`isTicketLink`, `isMailLink`). The only links the HUD renders (`sanitise.ts` unwraps every other `<a>`) |
