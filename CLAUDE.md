...
| `bridge/holdings.mjs` / `src/ui/Holdings.tsx` | The Robinhood holdings card (`JARVIS_HOLDINGS=robinhood`): an `askReadOnly` job given the Robinhood connector alone (`connectors`, `allowWith`) and only the tools in `HOLDINGS_TOOLS` (`allowTool`); each figure is checked against the raw tool results (`onToolResult`, `verify`) and totals are summed in the bridge. Sent as a `holdings` frame when a page opens; drawn from data by `Holdings.tsx` (blade kind `holdings`), never model markup; `src/lib/holdings.ts` formats it |
