...
const { bringsContent, conversationDisallowed, decideTool, egressGate, intentGate } = await import('../bridge/policy.mjs')
  ...
  const session = query({
    ...
    options: {
      ...
      canUseTool: async (name, input) => {
        ...
        const verdict = egressGate(name, intentGate(name, decideTool(name, POLICY), c.say, POLICY), { tainted, input }, POLICY)
